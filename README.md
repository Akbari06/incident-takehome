...
My Approach
Parsed command-line arguments and input files using argparse and json.
Generated a repeating base schedule using the start date, interval, and user list.
Detected overlapping overrides and replaced affected time ranges.
Split original shifts to insert overrides while keeping unaffected parts intact.
Truncated schedule entries to fit within the --from and --until time window.
Output the final schedule as formatted JSON with UTC timestamps.
